...
app.get('/api/admin/kpis', async (req, res) => {
  try {
    ...
    if (supabaseAdmin) {
      // Creators
      const { count: activeCreators } = await supabaseAdmin
        .from('creator_profiles')
...
async function startServer() {
  if (process.env.NODE_ENV !== 'production' && !process.env.VERCEL) {
    ...
    try {
      ...
      console.error('[Server] Failed to load Vite middleware:', err);
    }
  } else {
    // Production mode or Vercel: Serve static output from Vite build
